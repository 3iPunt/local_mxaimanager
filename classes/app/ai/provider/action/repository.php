...
namespace local_mxaimanager\app\ai\provider\action;
...
class repository extends \local_mxaimanager\app\repository
{
    ...
    {
        return 'local_mxaimanager_provider_actions';
    }

    public function get_by_id(int $id): entity
    {
        return $this->base_factory->ai()->provider()->action()->entity(
        ...
        return $this->base_factory->collection(
            array_map(
                function (object $record) {
                    return $this->base_factory->ai()->provider()->action()->entity((array)$record);
